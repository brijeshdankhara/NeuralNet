...
class ClDataSet:
    ...
    def __init__(self):
        ...
        content_list = []
        imgvec = []
        dir = "/Users/brijeshdankhara/PycharmProjects/NeuralNet/mnist_images/"
        for content in os.listdir(dir):  # "." means current directory
            ...
            a = img
            imgvec.append(img)
        s = np.reshape(np.array(imgvec),(1000,784))

        #print np.array(img)
        tarvec = []
        for content in os.listdir("./mnist_images/"):
            if content[:1] == '0':
        ...
        self.samples = np.array(list1_shuf)/225
        self.samples = self.samples.T
        self.targets = np.array(list2_shuf)
...
nn_experiment_default_settings = {
    ...
    "layers_specification": [{"number_of_neurons": 10, "activation_function": "hardlimit"}],  # list of dictionaries
    "data_set": ClDataSet(),#.make_sample_and_target_sets(),
    ...
    def adjust_weights(self):
        self.neural_network.adjust_weights(self.data_set.samples,
                                           self.desired_target_vectors - self.neural_network.calculate_output(
                                               self.data_set.samples), self.desired_target_vectors,self.neural_network.calculate_output(
...
class ClNNGui2d:
    ...
    def __init__(self, master, nn_experiment):
        ...
        self.learning_method_dropdown.grid(row=4,column=0,sticky=Tk.N+Tk.E+Tk.S+Tk.W)
        self.initialize()
    ...
    def learning_method_dropdown_callback(self):
        ...
        self.nn_experiment.learning_method = self.learning_method
        self.nn_experiment.neural_network.learning_method = self.learning_method
...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error, target, active):
        num_of_samples = 1000
        num_of_input = 784
        #input_samples = input_samples.T
        lr = self.learning_rate;
        count = 0
        for (a1, a2) in zip(target, active):
            if a1[0] == a2[0]:
                count += 1
        error_percent = count
        if self.learning_method == 'Delta Rule':
            for layer_index, layer in enumerate(self.layers):
                update2 = np.ones((num_of_input+1,num_of_samples))
                update2[:-1, :] = input_samples
                newvec = []
                for (a1, a2) in zip(target, active):
                    newvec.append(a1-a2);
                update = np.dot(newvec, update2.T)
                layer.weights = layer.weights + lr*update
                print error_percent
                #print layer.weights
            ...
            for layer_index, layer in enumerate(self.layers):
                update2 = np.ones((num_of_input+1,num_of_samples))
                update2[:-1, :] = input_samples
                update = np.dot(target, update2.T)
                layer.weights = (1-lr)*layer.weights + lr*update
                print error_percent
                #print layer.weights
                return layer.weights
            ...
            for layer_index, layer in enumerate(self.layers):
                update2 = np.ones((num_of_input+1,num_of_samples))
                update2[:-1, :] = input_samples
                update = np.dot(active, update2.T)
                layer.weights = layer.weights + lr*update
                print error_percent
                #print layer.weights
                return layer.weights
