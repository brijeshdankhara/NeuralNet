...
alpha = 0.0003 # Note: if you want to adjust alpha during training, use a symbolic/shared variable
...
a1 = T.nnet.sigmoid(net1) # sigmoid
net2 = T.dot(a1,second_W) + b2
a2 = T.nnet.sigmoid(net2) # sigmoid

# Define our performance function
loss = abs(T.mean(T.nnet.categorical_crossentropy(a2, t)))
...
plt.title("NNet")
plt.scatter(0, 0)
plt.xlim(0, 50)#number of epoch
plt.ylim(0, 5)#error plot
mean_err = 0
mean_list = []
j_list = []
