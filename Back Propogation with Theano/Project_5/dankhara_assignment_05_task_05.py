...
def load_samples_targets():
    ...
    content_list = []
    imgvec = []
    dir = "/Users/brijeshdankhara/PycharmProjects/NeuralNet/train_large/"
    for content in os.listdir(dir):  # "." means current directory
        ...
        a = img
        imgvec.append(img)
    s = np.reshape(np.array(imgvec), (10000, 3072))
    tarvec = []
    for content in os.listdir("./train_large/"):
        if content[:1] == '0':
    ...
    content_list1 = []
    imgvec1 = []
    dir = "/Users/brijeshdankhara/PycharmProjects/NeuralNet/test_large/"
    for content in os.listdir(dir):  # "." means current directory
        ...
        a = img
        imgvec1.append(img)
    s1 = np.reshape(np.array(imgvec1), (1000, 3072))
    tarvec1 = []
    for content in os.listdir("./test_large/"):
        if content[:1] == '0':
...
first_W = theano.shared(np.random.uniform(-0.0001, 0.0001, (3072, 50)), name='Layer1weights')
second_W = theano.shared(np.random.uniform(-0.0001, 0.0001, (50, 10)), name='Layer2weights')
...
mean_list = []
j_list = []
for j in range(50): ## EPOCH
    for i in range(10000): ## number of images in the dataset
        err, output, net2, W1, W2 = train(samples[:,i:i+1], targets[:,i:i+1].T)
        if j % 5 == 0:
            mean_err = err + mean_err
    if j % 5 == 0:
        j_list.append(j)
        mean_list.append(mean_err/((j+1)*10000))
